...
<DemoStatus
  title="Live demo · LangExtract source-grounded extraction"
  description="The interactive demo replays the real notebook texts: the 2025-12-22 三机构 news brief from cell 66 (6 entity classes, 16 extractions) and the Romeo & Juliet few-shot from cell 107 (3 classes with per-extraction attributes). Hover any highlight to see the actual char_interval [start-end] LangExtract returns; the long-doc preset (54k chars → 1,889 extractions, 3 passes, 20 workers) is the project's real production config."
/>
