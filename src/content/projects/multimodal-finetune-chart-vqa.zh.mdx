...
<DemoStatus
  title="Demo 真实材料对应"
  description="互动 Demo 用同一张中文图表 + 同一个问题，对比通用 VLM 和微调后模型：通用模型读不准中文标签，微调模型给出精确标签 + 数值。微调答案取自 llamafactory_train.jsonl 的真实 assistant 目标，训练命令是真实 LlamaFactory 设置，浏览器里不跑模型。"
/>
