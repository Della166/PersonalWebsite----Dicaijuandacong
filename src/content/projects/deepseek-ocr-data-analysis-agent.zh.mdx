...
<DemoStatus
  title="Demo 真实材料对应"
  description="互动 Demo 在一张样例财报表格上复演三层管线：ocr_service 解析出带坐标的单元格 → analysis_service 算 KPI + 标异常 → visualization_service 让 LLM 选图表类型再渲染。展示的是 core/ocr · core/analysis · core/visualization 每层真实行为，不调真实 vLLM/LLM。"
/>
